...
subtitle: Accompanying 'PHENOMICL_downstream'
...
# AEDB <- haven::read_sav(paste0(AEDB_loc, "/2022_1_NEW_AtheroExpressDatabase_ScientificAE_11-02-2022.sav"))
# AEDB <- haven::read_sav(paste0(AEDB_loc, "/2022_1_NEW_AtheroExpressDatabase_ScientificAE_15-02-2022.sav"))
# AEDB <- haven::read_sav(paste0(AEDB_loc, "/2022_3_NEW_AtheroExpressDatabase_ScientificAE_27-09-2022.sav"))
...
# 3	Major stroke			
# 4	Amaurosis fugax			
# 5	Four vessel disease			
# 7	Vertebrobasilary TIA			
# 8	Retinal infarction			
# 9	Symptomatic, but aspecific symtoms
# 10	Contralateral symptomatic occlusion			
# 11	retinal infarction			
# 12	armclaudication due to occlusion subclavian artery, CEA needed for bypass			
# 13	retinal infarction + TIAs			
# 14	Ocular ischemic syndrome			
# 15	ischemisch glaucoom			
# 16	subclavian steal syndrome			
# 17	TGA
...
# 
table(AEDB$sympt, AEDB$AsymptSympt2G, useNA = "ifany")
table(AEDB$sympt, AEDB$Symptoms.5G, useNA = "ifany")
table(AEDB$AsymptSympt2G, 
      AEDB$Symptoms.5G, useNA = "ifany")
...
There are few hormones or precursors thereoff that we are interested in. These are:

- Estradiol, pmol/L, measured at LKCH 2016
- Testosterone, nmol/L, measured at LKCH 2016
- SHB, nmol/L, measured at LKCH 2016

First, we will prepare the variables. Consider also that a few values are not valid:

-777	- below detection limit			
-666	- too little material

We must make sure that these are coded as missing values.

```{r}
# Create new variables out of the original
AEDB$Estradiol_plasma <- as.numeric(AEDB$Estradiol) 
AEDB$Testosterone_plasma <- as.numeric(AEDB$Testosterone) 
AEDB$SHBG_plasma <- as.numeric(AEDB$SHBG) 

# Fix the missing values
AEDB$Estradiol_plasma[AEDB$Estradiol_plasma == -777] <- NA
AEDB$Estradiol_plasma[AEDB$Estradiol_plasma == -666] <- NA
AEDB$Testosterone_plasma[AEDB$Testosterone_plasma == -777] <- NA
AEDB$Testosterone_plasma[AEDB$Testosterone_plasma == -666] <- NA
AEDB$SHBG_plasma[AEDB$SHBG_plasma == -777] <- NA
AEDB$SHBG_plasma[AEDB$SHBG_plasma == -666] <- NA

# Convert the unit for Estradiol to nmol/L
AEDB$Estradiol_plasma <- AEDB$Estradiol_plasma * 0.001

# Rank normalize the new variables
AEDB$Estradiol_plasma_rankNorm <- qnorm((rank(AEDB$Estradiol_plasma, na.last = "keep") - 0.5) / sum(!is.na(AEDB$Estradiol_plasma)))
AEDB$Testosterone_plasma_rankNorm <- qnorm((rank(AEDB$Testosterone_plasma, na.last = "keep") - 0.5) / sum(!is.na(AEDB$Testosterone_plasma)))
AEDB$SHBG_plasma_rankNorm <- qnorm((rank(AEDB$SHBG_plasma, na.last = "keep") - 0.5) / sum(!is.na(AEDB$SHBG_plasma)))

```

Second, we want to create a histogram using `ggpubr` for each of the hormones prior to and after inverse-rank normal transformation.
...
# Create a for loop to create the histograms and make sure that the display is within the for loop, as is the combined for loop. 
hormone_list <- c("Estradiol_plasma", "Testosterone_plasma", "SHBG_plasma")
hormone_rankNorm_list <- c("Estradiol_plasma_rankNorm", "Testosterone_plasma_rankNorm", "SHBG_plasma_rankNorm")

# Loop through the raw and rank-normalized hormones
for (i in seq_along(hormone_list)) {
  HORMONE <- hormone_list[i]
  HORMONE_rankNorm <- hormone_rankNorm_list[i]
  
  # Rename variables for display in title and xlab
  if (HORMONE == "Estradiol_plasma") {
    HORMONE_name <- "Estradiol"
  } else if (HORMONE == "Testosterone_plasma") {
    HORMONE_name <- "Testosterone"
  } else if (HORMONE == "SHBG_plasma") {
    HORMONE_name <- "SHBG"
  }
  
  # Raw hormone data histogram
  p1 <- ggpubr::gghistogram(AEDB, HORMONE, 
                            color = "white",
                            fill = "Gender",
                            palette = c("#1290D9", "#DB003F"), 
                            add = "median", 
                            rug = TRUE,
                            title = paste0(HORMONE_name),
                            xlab = paste0(HORMONE_name, " in citrate plasma (nmol/L)"),
                            ggtheme = theme_minimal())
  
  # Rank-normalized hormone data histogram
  p2 <- ggpubr::gghistogram(AEDB, HORMONE_rankNorm, 
                            color = "white",
                            fill = "Gender",
                            palette = c("#1290D9", "#DB003F"), 
                            add = "median", 
  ...
  # Display combined graphs for each hormone
  print(p1 / ggpar(p2, legend = "none"))
}
```
...
AEDB$Estradiol_plasma <- as.numeric(AEDB$Estradiol_plasma)
AEDB$Testosterone_plasma <- as.numeric(AEDB$Testosterone_plasma)
AEDB$SHBG_plasma <- as.numeric(AEDB$SHBG_plasma)
AEDB$Estradiol_plasma_rankNorm <- as.numeric(AEDB$Estradiol_plasma_rankNorm)
AEDB$Testosterone_plasma_rankNorm <- as.numeric(AEDB$Testosterone_plasma_rankNorm)
AEDB$SHBG_plasma_rankNorm <- as.numeric(AEDB$SHBG_plasma_rankNorm)
...
AEDB$indexsymptoms_latest_4g <- to_factor(AEDB$indexsymptoms_latest_4g)
...
basetable_vars = c("Hospital", "ORyear",
                   ...
                   "indexsymptoms_latest_4g", "Symptoms.Update2G", "Symptoms.Update3G",
                   "restenos", "stenose",
                   ...
                   "Estradiol_plasma", "Testosterone_plasma", "SHBG_plasma",
                   "Estradiol_plasma_rankNorm", "Testosterone_plasma_rankNorm", "SHBG_plasma_rankNorm")
...
## ExpressScan - Time to Event

We check what the relation is between symptom type and the time to event. This is based on the 'latest' symptoms grouped in 4 groups (check *Re-assessed symptoms*).

```{r}
library(sjlabelled)
AEDB.CEA$TimeOR_latest_4g <- as_numeric(AEDB.CEA$TimeOR_latest_4g)
summary(AEDB.CEA$TimeOR_latest_4g)
```


```{r}
attach(AEDB.CEA)
AEDB.CEA[,"TimeOR_latest4G"] <- AEDB.CEA$TimeOR_latest_4g
AEDB.CEA$TimeOR_latest4G[TimeOR_latest_4g == -999] <- NA
AEDB.CEA$TimeOR_latest4G[TimeOR_latest_4g == -888] <- NA
AEDB.CEA$TimeOR_latest4G[TimeOR_latest_4g < 0] <- NA
detach(AEDB.CEA)

summary(AEDB.CEA$TimeOR_latest4G)

```

```{r}
compare_means(TimeOR_latest4G ~ indexsymptoms_latest_4g, data = AEDB.CEA %>% filter(!is.na(indexsymptoms_latest_4g)), method = "kruskal.test")
ggpubr::ggboxplot(AEDB.CEA %>% filter(!is.na(indexsymptoms_latest_4g)), 
                  x = c("indexsymptoms_latest_4g"),
                  y = "TimeOR_latest4G", 
                  xlab = "latest 'index' event prior to CEA",
                  ylab = "time between 'index' event and CEA",
                  color = "indexsymptoms_latest_4g",
                  palette = "npg",
                  add = "jitter") +
  stat_compare_means(aes(group = indexsymptoms_latest_4g), label = "p.format", method = "kruskal.test")

```



## ExpressScan - CONVOCALS sample list

Here we create the CONVOCALS sample list to match/cross reference with the whole-slide images. This includes:

- sampleID = STUDY_NUMBER
- gender 

```{r}
library(data.table)
require(labelled)
CONVOCALS.samplelist.withduplicates <- subset(AEDB.CEA, select = 
                                                c("STUDY_NUMBER", "UPID", "informedconsent", "Hospital", 
                                                  "Artery_summary", "arteryop", "ORyear", 
                                                  ...
                                                  "OverallPlaquePhenotype", "Plaque_Vulnerability_Index",
                                                  "EP_composite", "EP_composite_time", "EP_major", "EP_major_time",
                                                  "sympt", "AsymptSympt", "AsymptSympt2G",  "Symptoms.5G", 
                                                  "indexsymptoms_latest_4g", "Symptoms.Update2G", "Symptoms.Update3G", "TimeOR_latest4G", # new symptom-categories
                                                  "Estradiol_plasma", "Testosterone_plasma", "SHBG_plasma", # raw hormones
                                                  "Estradiol_plasma_rankNorm", "Testosterone_plasma_rankNorm", "SHBG_plasma_rankNorm") # rank normalized hormones
                                              )

CONVOCALS.samplelist.withduplicates$arteryop <- to_factor(CONVOCALS.samplelist.withduplicates$arteryop)
# library(DT)
# DT::datatable(CONVOCALS.samplelist.withduplicates)

```

### Samples vs. individuals

The AE is using `STUDY_NUMBER` to mark each _individual sample_, but `UPID` numbers are used for each _individual patient_. The latter is important for studies involving, for example genetic data, or in this case predictions. 

Here we assess how many UPIDs there are in the sample-list and how many of these are 'duplicate', _i.e._ how many patients had multiple surgeries. Next, we randomly select one of each 'multiple' UPID.

The general flow is: 

* Select samples and store in a dataset `CONVOCALS.samplelist.withduplicates`
* Randomly select 1 sample from `CONVOCALS.samplelist.withduplicates` based on UPIDs
* Create new dataset `CONVOCALS.samplelist`

```{r}
# what is the damage?

n_occur1 <- as.data.frame(table(CONVOCALS.samplelist.withduplicates$UPID))
dim(n_occur1)
n_occur1a <- n_occur1[n_occur1$Freq>1,]
rm(n_occur1)
n_occur1a

# function to filter
# reference: https://stackoverflow.com/questions/23084509/function-for-sampling-between-duplicated-values-in-data-frame
# this is not randomly picking one sample
# filter_data <- function(DATA){
#   distinct_points <- unique(DATA$UPID)
#   as.data.frame(t(sapply(distinct_points, function(x){subset(DATA, UPID == x)[1,]})))
# }
# 
# CONVOCALS.samplelist <- filter_data(CONVOCALS.samplelist.withduplicates)

# Reference: https://stackoverflow.com/questions/45804730/randomly-remove-duplicated-rows-using-dplyr

library(dplyr)
set.seed(9192202)
CONVOCALS.samplelist <- CONVOCALS.samplelist.withduplicates %>% 
  group_by(UPID) %>%
  sample_n(1)
...
fwrite(CONVOCALS.samplelist,
       file = paste0(OUT_loc, "/", Today, ".CONVOCALS.samplelist.csv"),
       quote = TRUE,
       sep = ",",
       na = "NA",
       row.names = FALSE,
...
```{r}
EVG_slides <- openxlsx::read.xlsx(paste0(STAINS_loc, "/20231023_STAINS_on_HPC.xlsx"),
                                      sheet = "EVG",
                                      colNames = TRUE)
EVG_slides$Order <- NULL
head(EVG_slides)

```

```{r}
CONVOCALS.samplelist.EVG <- merge(CONVOCALS.samplelist, EVG_slides, by.x = "STUDY_NUMBER", by.y = "STUDY_NUMBER", all.x = TRUE)

```

```{r}
table(CONVOCALS.samplelist.EVG$STAIN, useNA = "ifany")

table(CONVOCALS.samplelist.EVG$FileType, useNA = "ifany")

table(CONVOCALS.samplelist.EVG$indexsymptoms_latest_4g, useNA = "ifany")

table(CONVOCALS.samplelist.EVG$Symptoms.Update3G, useNA = "ifany")

table(CONVOCALS.samplelist.EVG$Artery_summary, useNA = "ifany")
```

```{r}
fwrite(subset(CONVOCALS.samplelist.EVG, is.na(STAIN)),
       file = paste0(OUT_loc, "/", Today, ".CONVOCALS.samplelist.noEVGslides.csv"),
       quote = TRUE,
       sep = ",",
       na = "NA",
       row.names = FALSE,
...
fwrite(CONVOCALS.samplelist.EVG,
       file = paste0(OUT_loc, "/", Today, ".CONVOCALS.samplelist.withEVGslides.csv"),
       quote = TRUE,
       sep = ",",
       na = "NA",
       row.names = FALSE,
...
openxlsx::write.xlsx(CONVOCALS.samplelist.EVG,
       file = paste0(OUT_loc, "/", Today, ".CONVOCALS.samplelist.withEVGslides.xlsx"),
       overwrite = TRUE,
       sheetName = "EVG_SampleList",
       tabColour = uithof_color[16],
...
> Note: we have 17 samples where there are two pieces of the segments, or there are two scans. These are:
> - 292	UPID00725	AE292.EVG.TIF
> - 292	UPID00725	AE292.EVG.v2.ndpi
> - 297	UPID00329	AE297.EVG.TIF
> - 297	UPID00329	AE297.EVG.v2.ndpi
> - 300	UPID01807	AE300.EVG.TIF
> - 300	UPID01807	AE300.T03-24318.EVG.v2.ndpi
> - 363	UPID00682	AE363.T04-09070.EVG.B.ndpi
> - 363	UPID00682	AE363.T04-09070.EVG.A.ndpi
> - 926	UPID00593	AE926.EVG.TIF
> - 926	UPID00593	AE926.T05-10375.EVG.v2.ndpi
> - 1565	UPID00947	AE1565.T07-14473.EVG.v2.ndpi
> - 1565	UPID00947	AE1565.EVG.TIF
> - 1645	UPID00700	AE1645.T07-10464.EVG.v2.ndpi
> - 1645	UPID00700	AE1645.EVG.TIF
> - 1671	UPID00420	AE1671.EVG.TIF
> - 1671	UPID00420	AE1671.T07-15965.EVG.v2.ndpi
> - 1672	UPID00688	AE1672.T07-15706.EVG.v2.ndpi
> - 1672	UPID00688	AE1672.EVG.TIF
> - 1676	UPID00083	AE1676.T07-13742.EVG.v2.ndpi
> - 1676	UPID00083	AE1676.EVG.TIF
> - 1811	UPID01881	AE1811.T07-14002.EVG.v2.ndpi
> - 1811	UPID01881	AE1811.EVG.TIF
> - 2039	UPID01975	AE2039.T08-20781.EVG.v2.ndpi
> - 2039	UPID01975	AE2039.EVG.TIF
> - 2040	UPID01976	AE2040.EVG.TIF
> - 2040	UPID01976	AE2040.T08-20757.EVG.v2.ndpi
> - 2083	UPID02217	AE2083.EVG.TIF
> - 2083	UPID02217	AE2083.T09-16562.EVG.v2.ndpi
> - 2289	UPID02336	AE2289.EVG.v2.ndpi
> - 2289	UPID02336	AE2289.EVG.TIF
> - 2298	UPID02363	AE2298.EVG.v2.ndpi
> - 2298	UPID02363	AE2298.EVG.TIF
> - 2343	UPID00927	AE2343.T09-31738.EVG.v2.ndpi
> - 2343	UPID00927	AE2343.EVG.TIF
> 
> We need to choose which samples to use.


## HE samples

```{r}
HE_slides <- openxlsx::read.xlsx(paste0(STAINS_loc, "/20231023_STAINS_on_HPC.xlsx"),
                                      sheet = "HE",
                                      colNames = TRUE)
HE_slides$Order <- NULL
head(HE_slides)

```

```{r}
CONVOCALS.samplelist.HE <- merge(CONVOCALS.samplelist, HE_slides, by.x = "STUDY_NUMBER", by.y = "STUDY_NUMBER", all.x = TRUE)

```

```{r}
table(CONVOCALS.samplelist.HE$STAIN, useNA = "ifany")

table(CONVOCALS.samplelist.HE$FileType, useNA = "ifany")

table(CONVOCALS.samplelist.HE$indexsymptoms_latest_4g, useNA = "ifany")

table(CONVOCALS.samplelist.HE$Symptoms.Update3G, useNA = "ifany")

table(CONVOCALS.samplelist.HE$Artery_summary, useNA = "ifany")
```

```{r}
fwrite(subset(CONVOCALS.samplelist.HE, is.na(STAIN)),
       file = paste0(OUT_loc, "/", Today, ".CONVOCALS.samplelist.noHEslides.csv"),
       quote = TRUE,
       sep = ",",
       na = "NA",
       row.names = FALSE,
...
fwrite(CONVOCALS.samplelist.HE,
       file = paste0(OUT_loc, "/", Today, ".CONVOCALS.samplelist.withHEslides.csv"),
       quote = TRUE,
       sep = ",",
       na = "NA",
       row.names = FALSE,
...
openxlsx::write.xlsx(CONVOCALS.samplelist.HE,
       file = paste0(OUT_loc, "/", Today, ".CONVOCALS.samplelist.withHEslides.xlsx"),
       overwrite = TRUE,
       sheetName = "HE_SampleList",
       tabColour = uithof_color[16],
       creator = "Sander W. van der Laan",
       headerStyle = hs,
       keepNA = TRUE, na.string = "NA")
...
> Note: we have 3 samples where there are two pieces of the segments. These are:
> - 1	UPID90067 AE1.T02-5340.B.HE.TIF AE1.T02-5340.A.HE.TIF
> - 363	UPID00682 AE363.T04-09070.HE.B.ndpi AE363.T04-09070.HE.A.ndpi
> - 921	UPID01421 AE921.T05-16731.HE.A.ndpi AE921.T05-16731.HE.B.ndpi


## SMA samples

```{r}
SMA_slides <- openxlsx::read.xlsx(paste0(STAINS_loc, "/20231023_STAINS_on_HPC.xlsx"),
                                      sheet = "SMA",
                                      colNames = TRUE)
SMA_slides$Order <- NULL
head(SMA_slides)
```

```{r}
CONVOCALS.samplelist.SMA <- merge(CONVOCALS.samplelist, SMA_slides, by.x = "STUDY_NUMBER", by.y = "STUDY_NUMBER", all.x = TRUE)

```

```{r}
table(CONVOCALS.samplelist.SMA$STAIN, useNA = "ifany")

table(CONVOCALS.samplelist.SMA$FileType, useNA = "ifany")

table(CONVOCALS.samplelist.SMA$indexsymptoms_latest_4g, useNA = "ifany")

table(CONVOCALS.samplelist.SMA$Symptoms.Update3G, useNA = "ifany")

table(CONVOCALS.samplelist.SMA$Artery_summary, useNA = "ifany")
```

```{r}
fwrite(subset(CONVOCALS.samplelist.SMA, is.na(STAIN)),
       file = paste0(OUT_loc, "/", Today, ".CONVOCALS.samplelist.noSMAslides.csv"),
       quote = TRUE,
       sep = ",",
       na = "NA",
       row.names = FALSE,
...
fwrite(CONVOCALS.samplelist.SMA,
       file = paste0(OUT_loc, "/", Today, ".CONVOCALS.samplelist.withSMAslides.csv"),
       quote = TRUE,
       sep = ",",
       na = "NA",
       row.names = FALSE,
...
openxlsx::write.xlsx(CONVOCALS.samplelist.SMA,
       file = paste0(OUT_loc, "/", Today, ".CONVOCALS.samplelist.withSMAslides.xlsx"),
       overwrite = TRUE,
       sheetName = "SMA_SampleList",
       tabColour = uithof_color[16],
       creator = "Sander W. van der Laan",
       headerStyle = hs,
       keepNA = TRUE, na.string = "NA")
...
------------------------------------------------------------------------------------------------------------------------
    ...
    Minimum requirements: R version 4.3.1 (2023-06-16) -- "Beagle Scouts", Mac OS X Sonoma (14.0, beta 5, 23A5312d)
    ...
    **Changes log**
    * v1.10.0 Added sex-hormones raw and rank-normalized Estradiol, Testosterone, and SHBG.
    * v1.0.9 Created baseline table for CONVOCALS. Changed the organisation of CONVOCALS: one GitHub repository is for the (generic) MIL-model-code, the other for the CONVOCALS-project specific code.
    * v1.0.8 Added EVG files, type, filename, and stain information.
    * v1.0.7 Check presence SMA and HE files, including type, filename, and stain. Added Excel-export.
    * v1.0.6 Added time-to-event analysis with respect to symptoms. Add this to the output table.
    * v1.0.5 Added new SMA slides. Added baseline characteristics data.
    * v1.0.4 Updated AE database. Output includes IPH. Created list of samples without SMA slides.
    * v1.0.3 Updated AE database. Added function to randomly select multiplicate UPIDs.
