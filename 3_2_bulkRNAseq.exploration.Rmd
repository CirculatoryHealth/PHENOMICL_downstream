...
```{r}
AERNAScomboSE <- readRDS(file = paste0(OUT_loc, "/",Today,".AERNAScomboSEvst.CEA.1092pts.SE.after_qc.IC_academic.RDS"))
# AERNAScomboSE <- readRDS(file = paste0(OUT_loc, "/20240109.AERNAScomboSEvst.CEA.1092pts.SE.after_qc.IC_academic.RDS"))
...
expression_wide2 <- expression_wide %>%
  ...
  dplyr::select(study_number, StudyName, Gender, symbol_ensembl_gene_list_qc, expression_value_sample)
...
expression_long2 <- expression_wide2 %>%
  ...
  dplyr::select(-symbol_ensembl) %>%
  dplyr::mutate(gene = factor(gene, levels = c("Random genes", gene_list_qc)))
expression_long2
...
p3 <- ggpubr::ggboxplot(expression_long2,
  ...
  ggsave(filename = paste0(PLOT_loc, "/", Today, ".TargetExpression_vs_1000genes_by_Gender.png"), plot = last_plot())
  ggsave(filename = paste0(PLOT_loc, "/", Today, ".TargetExpression_vs_1000genes_by_Gender.pdf"), plot = last_plot())
  ggsave(filename = paste0(PLOT_loc, "/", Today, ".TargetExpression_vs_1000genes_by_Gender.ps"), plot = last_plot())
  
  rm(p3)
```

List all the `gene` for all unique genes where their mean expression is higher than the mean of the 1,000 random genes (`mean_1000_genes`).
...
expression_long2_high
```

List all genes with expression lower than `mean_1000_genes`.
...
expression_long2_low
```

List all genes where the mean of their expression is higher or lower than the mean of 1,000 random genes, and where there is a statistical difference between gender.
...
saveWorkbook(wb, paste0(OUT_loc, "/", Today,".iph_area_075_vst_dgea_gender_significant_genes.xlsx"), overwrite = TRUE)
...
temp.cor <- cor(temp2, method = "spearman")
```
...
saveRDS(AERNAScomboSE_target, file = paste0(OUT_loc, "/", Today, ".AERNAScomboSE_target.CEA.1092pts.SE.after_qc.IC_academic.RDS"))
